...
The M7 Pattern path is planner → deterministic plan validation → writer → deterministic candidate validation → independent semantic verifier → guarded publication. `PATTERN_AI_ROLLOUT` remains `off` in both committed Wrangler environments; implemented provider code is not evidence of deployment, credential configuration, ontology activation, or a completed canary.
...
- **Q1 uses inclusive, differently scoped ceilings:** at most 2 planner calls per job, 3 writer calls per job against one frozen plan, and 2 verifier calls per candidate. The worst case is `2 + 3 + (3 × 2) = 11`, not 7 or 14. `isPatternCommand` validates the frozen maxima, each pass checks its zero-based next-attempt index before any provider call, and `advance(..., "semantic_verifying")` resets `verifier_attempts` for the new candidate.
- **No transition writes the counter of the pass whose result it is committing.** Every counter write commits in the same guarded D1 batch as the transition that authorizes the next call. `advance` commits success without incrementing that pass; `retryStage` increments the same pass for a genuine retry; `returnToWriter` crosses from verifier to writer and increments `writer_attempts`, not `verifier_attempts`. Do not summarize this as “`retryStage` is the sole incrementer.”
...
- The last recorded remote migration state predates `0009`, `0010`, and `0011`; re-query it rather than treating this file as live evidence. Apply `0011_ontology_pipeline_evidence.sql` before the first production API deploy that JOINs `pattern_ontology_pipeline_evidence`, after applying the adapter's reserved `0009` and `0010` in order. Bookmark and export first. A missing pipeline receipt continues to fail machine ingestion closed.

...
npm run db:local -w @patternlike/api   # apply ordered db/d1/ (0001–0022) to local D1
...
`contracts/m0/` is the frozen baseline (JSON Schema + OpenAPI + valid/invalid fixtures); additive milestone contracts run through `contracts/m8/`. `db/d1/0001_m0_core.sql` is the core operational schema. Product-spec v0.2 remains the baseline, v0.5 restates the daily-reading contract, and v0.6 adds AI-generated Pattern.
...
Order on the product API is `configGuard` → `authenticate` → `accountStateGate`. An `active` account without the current `account_processing` grant (`policy_version` `account-processing-v1-2026-08-28`) gets `403 account_processing_required` on every product path except the recovery set in `recoveryRouteAllowed`: `/v1/account`, `/v1/exports`, `/v1/exports/*`, `/v1/consents/ai-synthesis`, `/v1/consents/pattern-generation`, and `/v1/consents/account-processing`. A frozen account is limited to those same recovery routes (`403 account_not_active` elsewhere). Reading feedback and the M4/M6 privacy surfaces have real handlers. `routes/stubs.ts` is mounted last and contains only legacy fallback registrations shadowed by those handlers; do not use it as a feature-status inventory.
...
- The compiled Daily writer policy is `READING_PROMPT_VERSION` `1.0.2` (`services/reading-prompt.ts` / `services/reading-publisher.ts`), pinned as `OPENAI_READING_PROMPT_VERSION` in both Wrangler blocks. Voice and specificity are bounded synthesis guidance measured by the offline corpus; they do not change the packet, output schema, or validator. A Wrangler pin that disagrees with the compiled constant is `503 configuration_error`.
...
- **Pattern serves from an authored ontology, and that is a decision, not a gap.** `ontologyServesAccount` in `db/pattern-ontology.ts` admits `provenance.origin === "synthetic_internal"` on origin alone; only the `machine_pipeline` branch additionally requires `activationScope === "public"`. Do not "restore" this to machine-only. Fifteen machine candidate versions are logged in `docs/deploy/openai-pattern-rollout.md` and none ever passed the regressing stage, so the strict predicate closes Pattern for every reader rather than opening it — which is exactly what it did between `c21f615` and `794767f`. The origin is enumerated rather than defaulted so a third origin added later fails closed until someone decides what assurance it carries.
- **What the authored path costs is written down because it is not obvious.** An authored release skips the independent evaluator and the seven regression hard gates — `suppressed_feature_leak`, `uncited_astrological_claim`, `source_dependency_failure`, `prohibited_claim`, `mandatory_feature_omission`, `private_projection_leak`, `semantic_refusal` — and `evaluateOntologyRegressionHardGates` has exactly one non-test caller, inside the regression stage. Those checks therefore run **nowhere else in the product**: the live per-generation path has structural validation and a model semantic verifier, not a lexical suppression scan. What the authored release does carry is publication rights: every record is `source_supported` against a `licensed_excerpt` corpus and cites its fragment, it is signed by the isolated signer, and it compiles.
- **The machine ontology pipeline is parked, and shrinking its regression gate is a trap.** `ONTOLOGY_PIPELINE_ROLLOUT = "off"` in production; `build-internal-ontology.ts` supplies ontologies instead. If someone reopens the pipeline: `ONTOLOGY_REGRESSION_FIXTURE_COUNT` is duplicated as a hardcoded `30` inside the frozen command's *literal type* (`ontology-pipeline-command.ts`) and again in `commandIsExecutable`, and the 30 fixture documents are statically imported by **two** files — `ontology-regression.ts` and `ontology-regression-report.ts` — each with its own list. Changing the constant alone fails `loadOntologyRegressionCorpus` on a length mismatch. The count also cannot go below **7**: the corpus manifest declares 15 required coverage axes and no set of 5 or 6 fixtures covers them all.
...
- **Publication accepts one narrow proof, not an orchestrator-sized argument list.** `services/pattern-publication-proof.ts` rereads planner, validated-plan, writer, and semantic-verdict artifacts, recomputes their hashes, binds them to the durable job and frozen executed writer pin, and returns `PatternPublicationProof`. The publication batch rechecks account, active chart, confirmed locale revision, exact current Pattern consent, non-recalled ontology, reserved claim ownership, and running job before it inserts the document, accepts the claim, and succeeds the job.
...
- **Executed provenance is checked, not trusted.** `runPublisherPass` rejects metadata whose provider, pass, model, or prompt version differs from the frozen pin, and `provenanceFromExecutedPin` maps only the closed compiled writer-model allowlist into public `model_family`. Writer prompt `1.0.2` (`OPENAI_PATTERN_WRITER_PROMPT_VERSION`) is the warm, direct voice contract in `WRITER_POLICY`; planner stays `1.0.1` and the verifier stays `1.0.0-verifier` so the independence check in `verifierIndependenceProblem` still holds.
...
Place search is a separate, currently disabled control plane. `GEOCODER_ROLLOUT`
is a closed `off|enabled` var, committed `off` in both Wrangler blocks.
`POST /v1/places/{search,resolve}` and the geocoder consent GET return
`503 geocoder_unavailable` until it is `enabled`. Manual birthplace coordinates
remain the complete fallback. Migration `0022` is required before a Worker that
writes `place_resolutions`; that apply is **not** recorded on the production
ledger as of 2026-08-28 (0018–0020 are).

### D1 migration policy

M0 is pre-production, so `0001_m0_core.sql` is edited **in place** rather than superseded. Every statement is `CREATE TABLE IF NOT EXISTS`, so re-running it over an existing local database changes nothing and still exits 0. `npm run db:local` applies the ordered `db/d1/` directory (`0001`–`0022`), not only `0001`. After any schema or crypto-version change, delete and recreate:
...
Because `X-User-Id` names a user rather than creating one, every API test must `seedUser()` first — the `users` row, its `identities` row, and its wrapped DEK land together. `seedUser` also grants the current `account_processing` policy by default so ordinary fixtures model an authorized active account; consent/gate suites pass `{ accountProcessingConsent: false }` to exercise recovery. `confirmPreferences()`, `seedChart()`, and `seedActiveRelease()` supply the rest of what generation needs; a test that skips `confirmPreferences` gets a correct `timezone_confirmation_required` refusal rather than a reading.
...
- **Account-processing consent**: `POST /v1/birth-profiles` accepts only the exact current grant (`loadExactCurrentAccountProcessingGrant`), stored as `birth_profiles.consent_id`. Revocation from the privacy center freezes the account; regrant restores access. The local seed script does not grant this; tests do unless opted out.
...
- The remote ledger is recorded through **0020**. `0018_account_processing_consent.sql`, `0019_pattern_claim_transition_guards.sql`, and `0020_pattern_admin_sessions.sql` applied together on 2026-08-28 ~07:32 UTC before Worker version `fc7acdf3-cbfc-4724-8dbe-b127128e9637` (Access cutover; `PATTERN_ADMIN_TOKEN` already absent). `0016_birth_calc_usage.sql` applied 2026-08-27 ~06:35 UTC (6 commands) after an external mode-0600 SQL export and Time Travel bookmark; its birth guards are served by Worker version `287bce63-dece-4911-96db-dd212c2cec33`. `0017_codex_reading_provider.sql` applied 2026-08-27 ~17:32 UTC after its own export and bookmark; the Codex runner was stopped before that export and remained stopped at the recorded checkpoint. `0013` and `0014` applied 2026-08-24; `0015_ontology_pipeline_regression_evidence.sql` applied 2026-08-25 00:59:51 UTC; `0009` through `0012` applied in order on 2026-08-22. `0021_crypto_operations.sql` and `0022_place_resolutions.sql` are in the repository; no production apply is recorded for them. See `docs/deploy/openai-pattern-rollout.md` Gate 2 for the 0009–0015 recovery coordinates and Gate 9 for the 0018–0020 Access cutover. A missing pipeline evidence receipt still fails machine ingestion closed.
- **A Worker rollback does not land on a neutral earlier state.** Committed `[env.production]` now declares both crons (`*/15 * * * *` and `7,22,37,52 * * * *`), `READING_V5_ROLLOUT = "hybrid"`, and `READING_PUBLISHER = "codex"`. Rolling back to a pre-Codex Worker version still removes the Daily scheduler, privacy maintenance, and the Pattern sweep, and points Daily at a transport that no longer exists. Any `pipeline = 'reading'` rows left in D1 get no nudge repair and no artifact purge. Wrangler also warns that a rollback does not revert bound resources: an applied migration stays applied. Treat rollback as a distinct configuration, not as undo.

...
- **A mandatory uncertainty note the grammar cannot express fails closed at `prepareConstrainedReadingInput`, before the provider is paid.** The note is forced by a surviving *qualification* as well as a suppression, but the accepted disclosure forms are built only from `suppressed_features` and the approximate-time sentence — and `buildUncertainty()` qualifies `birthplace`/`birth_instant` at **every** accuracy, not just `approximate`. An exact-time chart with a qualified birthplace therefore has an empty suppression set and an unsatisfiable note; unguarded, `publisher_output_invalid` auto-replaces the command until the generation cap is spent. `uncertaintyDisclosureRepresentable` lives in `claim-support.ts` beside the grammar it mirrors, and the cross-check in `candidate-validation.test.ts` drives the real grammar so the two cannot drift — which is how this shipped, `contracts/m3/common.schema.json` having narrowed `qualifiedFeatureId` to `["moon","houses"]` from a partial read of the same function, with two unchecked `as` casts in `toAssemblyUncertainty` laundering the difference. This is a stop-gap: the qualification is still undisclosed, as it already is on approximate and unknown-time charts. See [`docs/deploy/daily-reading-uncertainty-disclosure.md`](docs/deploy/daily-reading-uncertainty-disclosure.md).

...
Two D1 pipelines share the Pattern surface (`#pattern`). Images (`0026`, `PATTERN_PORTRAIT_ENABLED`) write four `pattern_portrait_jobs` claimed at `/codex-provider/v1/portraits/*`. Meshes (`0027`, `PATTERN_PORTRAIT_MESH_ENABLED`) write `portrait_mesh_jobs` claimed at `/codex-provider/v1/portrait-meshes/*`. Mesh enablement requires the image flag and `ARTIFACTS`. Default wrangler `[vars]` omit both flags — only `[env.production.vars]` sets `"1"`. Only exact `"1"` enables; `PATTERN_GENERATION_ENABLED` does not gate this path.

Portrait is not a Cloudflare Queue. The D1 job row is the outbox; the runner polls (text → image → mesh, one claim at a time); `maintainPortraits` / `maintainPortraitMeshes` on the `*/15` lane recover leases and drain `portrait_start_outbox`. Creation requires live `account_processing` and `pattern_generation` grants plus an accepted four-chapter Pattern. Request `consent_policy_version` `1.0.0` is a typed confirm, not a new consent kind. Automation is a separate per-chart grant at policy `1.1.0`. Revoking those grants cancels unfinished work and retains `ready` portraits.
...
`GET /v1/readings?view=history|saved` is ungated beyond auth. Saves are revision-specific `(user_id, reading_id)` rows from `0028`; they hold no ciphertext. `PUT /v1/readings/:id/save` requires an empty body. `DELETE` is always `204`. Only `published|superseded|invalidated` rows with `reading_enc` are saveable. Account deletion deletes `reading_saves` before `daily_readings`. After saves exist, a rollback must keep Save-aware deletion.
...
- **`0029_daily_publication_receipts.sql` is in source and is not applied by the change that added it.** `MIGRATIONS.json` says apply it before a Worker that writes receipts. Constrained-model publication inserts one content-free row in the same guarded batch as the encrypted reading; deterministic V1 supplies `null`. Procedure: [`docs/deploy/release-attestation.md`](docs/deploy/release-attestation.md). Do not promote excluded artifact observations into this file.
...
- The last recorded operator observation of `content_releases` (2026-08-27, in `docs/deploy/openai-pattern-rollout.md`) found the table empty and `CONTENT_RELEASE_KEYS` unset. That is a dated inventory, not a live proof. Without an active release the legacy M3 daily-reading path answers `release_not_active`. This no longer affects Pattern, which reads an ontology rather than a content release, and M5 constrained-model daily readings do not depend on one either.

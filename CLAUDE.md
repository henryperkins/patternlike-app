...
Time Travel never touches `cycle_instances` or `cycle_passes`. Its only durable state is `cycle_scan_receipts`, keyed on the thirteen-column tuple in `0005` — including `receipt_epoch`.

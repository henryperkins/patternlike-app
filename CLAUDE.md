...
- The remote ledger is now at **0017**. `0016_birth_calc_usage.sql` applied 2026-08-27 ~06:35 UTC (6 commands) after an external mode-0600 SQL export and Time Travel bookmark; its birth guards are served by Worker version `287bce63-dece-4911-96db-dd212c2cec33`. `0017_codex_reading_provider.sql` applied 2026-08-27 ~17:32 UTC after its own export and bookmark; the Codex runner was stopped before that export and remained stopped at the recorded checkpoint. No compatible Worker that writes `reading`/`publisher` rows has been verified deployed. Earlier: `0013` and `0014` applied 2026-08-24; `0015_ontology_pipeline_regression_evidence.sql` applied 2026-08-25 00:59:51 UTC; `0009` through `0012` applied in order on 2026-08-22. `d1 migrations list` reports nothing pending, `foreign_key_check` is empty, `quick_check` is `ok`, and `assertion_probe` is empty. See `docs/deploy/openai-pattern-rollout.md` Gate 2 for the 0009–0015 recovery coordinates. A missing pipeline evidence receipt still fails machine ingestion closed.
...
- **`GEOCODER_ROLLOUT = "enabled"` makes `GOOGLE_MAPS_PLATFORM_API_KEY` load-bearing.** `checkSecureConfig` returns `503 configuration_error` on *every* request while the value is `enabled` and the secret is missing, so the secret goes onto the production Worker before the deploy that carries the value, never after. Rollback is a config commit flipping the value to `off` (`DELETE /v1/consents/geocoder` keeps working, so no grant is trapped); deleting the secret while the value is `enabled` takes the whole product down. The development block stays `off` — a local Worker enables the search through `.dev.vars` alongside the key. Gates, evidence, and the canary procedure are in `docs/deploy/geocoder-rollout.md`.

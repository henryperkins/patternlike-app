...
export interface Env {
  ...
  /**
   * Explicit route selection. Must be "codex" whenever the rollout is not off.
   *
   * Daily reaches its model through the durable Codex CLI runner and has no
   * fallback. `openai` remains a valid value in stored evidence -- it names the
   * publisher historical readings were generated by -- but it is not a route
   * this deployment can select.
   */
  READING_PUBLISHER?: string;
  /**
   * The `OPENAI_READING_*` names below are MODEL-FAMILY PINS, not proof of
   * direct API routing, and this Worker holds no OpenAI credential for Daily.
   * They are kept under their historical names because renaming a deployed
   * variable is cleanup rather than a prerequisite; the request reaches the
   * named model through the Codex runner.
   */
   ...
   * ceiling rather than what a synchronous Worker call could afford.
   */
   ...
   *
   * No longer read by Daily or Pattern. The ontology pipeline's OpenAI
   * transport is the only remaining consumer, and removing an unused key is a
   * separate staged production action because deleting a Worker secret deploys
   * a new version.
   */

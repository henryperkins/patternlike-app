...
await assertDatabaseHealthy(env.DB, "0031 clean apply");
...
// 0029–0031: upgrade the populated current 0028 schema without changing any prior
...
for (const [name, before] of beforeReceiptRows) {
  ...
  if (JSON.stringify(after.results) !== before) {
    throw new Error(`0029–0031 changed historical ${name} rows`);
  }
}

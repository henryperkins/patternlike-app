...
test("fictional canary uses the real pipeline with a fake provider and retains reproducible artifacts without secrets", async () => {
  ...
  const f = await jsonFixture("newversion", [program, audit]); const previousHome = process.env.CODEX_HOME;
  try {
    ...
    assert.equal(receipt.codex_cli_version, "0.154.0"); assert(receipt.compiled.triangles > 0);

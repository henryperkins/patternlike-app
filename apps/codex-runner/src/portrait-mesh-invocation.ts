...
export async function runPortraitMeshInvocation(options: PortraitMeshInvocationOptions): Promise<PortraitMeshInvocationOutcome> {
  ...
  try {
    const adaptive = claim.schema_version === "codex-portrait-mesh-claim/v2";
    const compilerVersion = adaptive ? PORTRAIT_MESH_V2_COMPILER_VERSION : PORTRAIT_MESH_COMPILER_VERSION;
    if (claim.compiler_version !== compilerVersion || !/^[A-Za-z0-9][A-Za-z0-9._:-]{0,99}$/.test(claim.model)
    ...
    const authored = await runIsolatedCodexJson({ ...shared, timeoutMs: remaining(), instructions: AUTHOR_INSTRUCTIONS,
      outputSchema: PORTRAIT_MESH_PROGRAM_SCHEMA, maxOutputBytes: 65536,
    ...
    const authoredProgram = parsePortraitMeshProgram(authored.value);
    if (!authoredProgram || authoredProgram.version !== "portrait-mesh-program/v1") return { ok: false, code: phase, fatal: false };
    const program: PortraitMeshProgram = adaptive ? { ...authoredProgram, version: "portrait-mesh-program/v2",
      chapter_count: claim.chapter_count, chapter_id: claim.chapter_id } : authoredProgram;
    remaining(); phase = "model_invalid";
    const identity = { chapterId: claim.chapter_id, documentRevision: claim.document_revision, sourceImageSha256: claim.source_image_sha256,
      sourceTextSha256: claim.source_text_sha256, ...(adaptive ? { chapterCount: claim.chapter_count } : {}) };
    ...
    const completion: CodexPortraitMeshCompletion = { lease_token: claim.lease_token, program, program_sha256: compiled.programSha256,
      ...
      ...(adaptive ? { schema_version: "codex-portrait-mesh-completion/v2" as const, chapter_count: claim.chapter_count,
        chapter_index: claim.chapter_index, chapter_id: claim.chapter_id, document_revision: claim.document_revision } : {}) } as CodexPortraitMeshCompletion;

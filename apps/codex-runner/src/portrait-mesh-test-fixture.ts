...
for await(const line of readline.createInterface({input:process.stdin})){
 ...
 if(r.method==='thread/start'){await fs.writeFile(path.join(root,'thread-'+count+'.json'),JSON.stringify(r.params));emit({id:r.id,result:{thread:{id:thread},model:r.params.model,modelProvider:'openai',sandbox:{type:mode==='sandbox'?'dangerFullAccess':'readOnly'},approvalPolicy:mode==='approvalpolicy'?'onRequest':'never'}});}

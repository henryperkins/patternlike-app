...
“Calculated, not invented” describes Pattern/Like's chart facts. Swiss Ephemeris establishes chart facts, the daily sky, and interpretation eligibility; a configured model writes the language of a daily reading under an explicit consent, from a closed set of supplied facts. User context may rank or frame a permitted interpretation but may never alter chart facts. Publication checks validate declared evidence links and the claim forms the validators recognize. Generated interpretation remains material for reflection and can be mistaken; a calculation does not establish it as a fact about a person's life. Generation records identify the configured model, prompt, and policy versions.
...
- Daily readings carry declared evidence links. Publication checks validate those links and recognized claim forms; they do not prove that every sentence follows from calculated evidence. A paragraph may use permitted personal context, which must not be presented as an astrological discovery.
- A reading is written by a configured model and published only after deterministic validation. There is no reviewed-copy fallback, no previous-day reuse, no alternate model, and no second model reviewing the first daily reading: a reading that cannot be produced and validated says so honestly instead. Your Pattern is a separate publication contract: a planner pass, a frozen plan, a writer pass, deterministic validation, and a separate semantic verifier pass, published only as a complete document. Passing these checks is not a guarantee that every generated claim is correct.
...
- `docs/superpowers/specs/2026-08-14-ai-generated-pattern-design.md`, as amended by `docs/superpowers/specs/2026-08-16-m7-spec-artifact-amendments.md`, defines the current Your Pattern product and engineering behavior. v0.5 remains the daily-reading contract. The v0.2 bundle is the superseded editorial-assembly baseline and is kept, unedited.
...
- `docs/deploy/consent-disclosures.md` records the September 2026 claim/consent copy correction, the source-backed application cleanup rules, and the provider-account evidence still needed.

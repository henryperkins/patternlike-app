...
The standalone desktop workspace pairs scene and reader (`1.85fr / 1fr`, reader minimum `300px`), with fluid gaps (`24px` to `46px`) and page gutters (`18px` to `52px`). The scene height follows available viewport space (`390px` to `590px`). The reader scrolls independently on desktop above a separate action footer, retaining position per chapter and perspective. The panel's available height accounts for the measured title and navigation rows; its sticky offset clears the account header, return bar and reading-mode strip. Complete reading uses a single centered measure (maximum `740px`).
...
Annotation placement observes text-size and wrapping changes without moving the camera. An occluded annotation or one that cannot fit clear of the controls defers to its named native chapter link; comparison also keeps annotations clear of both objects. The native link receives keyboard focus and Show in portrait actions when needed. Show in portrait selects and focuses the source annotation without issuing a camera command, retaining a manually adjusted view. It scrolls the page only as needed to reveal that link between the sticky navigation and account footer; it does not scroll the clipped label overlay or try to fit the entire canvas into a short window. Reduced-motion CSS disables explorer transitions completely so grid and text resizing cannot clamp the restored reading position.
...
The reader preserves chapter title, summary, all Overview paragraphs, Tensions, Resources, and Another expression. Comparison presents the same perspective in two chapters; the guide advances only through explicit Previous/Next actions. Each unchanged paragraph has a passage number, and its selected state uses a visible Selected in portrait caption and the reading's forest rule. Caption space stays reserved when selection changes to avoid moving the prose. The private live announcement includes each selected chapter, perspective and passage number. Show in portrait connects an unchanged passage to its scene annotation, and the annotation returns focus to that source passage. Original-image inspection retains its label, rationale, chapter title, and image reference.

On desktop, a distinct footer keeps original-image inspection and comparison outside the scrolling prose. More to read and Continue reading expose internal continuation; the action advances by eighty percent of the visible pane to retain overlap. At the end, Back to start returns to the beginning without changing chapter, perspective, passage selection, camera or navigation history. The continuation row reserves the larger wrapped label and button dimensions before scroll restoration, including while its overflow state is measured, and wrapped footer controls consume their own layout space. The reader is natively focusable for keyboard scrolling. When resizing removes internal scrolling, a focused continuation control hands focus to the reader. Narrow and comparison layouts keep page scrolling and place the action row after the reading, without the internal continuation row. Image, comparison and Full reading returns retain the originating passage and per-perspective scroll position. Comparison refreshes the scene's observed control rows and projection insets even when the canvas size is unchanged, preserving the manual camera view on return.

...
The saved Pattern leads the account page, ahead of the calculated chart facts. Its observatory opens automatically with one authored reading folio per chapter (three through six). Folios are reading furniture, not generated personal metaphors. Optional verified personal artwork replaces them when available; artwork status or consent never gates the room. The v1 generator remains limited to four chapters and explicit consent. Existing grants can still be disabled for other chapter counts. Complete reading, uncertainty, source identity, and private asset verification remain intact. The first opening does not move focus or scroll the page; explicit reading choices remain in session memory.

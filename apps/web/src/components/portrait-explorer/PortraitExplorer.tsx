...
export function PortraitExplorer({ source, objectBindings = EMPTY_BINDINGS, meshBundle, navigation }: {
  source: PortraitSource; objectBindings?: readonly PortraitObjectBinding[]; meshBundle: PortraitMeshBundle; navigation?: ExplorerNavigation;
}) {
  ...
  return navigation ? <ReadyExplorer key={identity} manifest={manifest} meshBundle={meshBundle} navigation={navigation} embedded />
    : <StandaloneExplorer key={identity} manifest={manifest} meshBundle={meshBundle} />;
}

function StandaloneExplorer(props: { manifest: PortraitManifest; meshBundle: PortraitMeshBundle }) {
...
function ReadyExplorer({ manifest, meshBundle, navigation, embedded = false }: { manifest: PortraitManifest; meshBundle: PortraitMeshBundle; navigation: ExplorerNavigation; embedded?: boolean }) {
  ...
  const scrollKey = state.presentation === "reading" ? `reading:${readerKey}` : state.presentation;
  ...
  const rememberScroll = useCallback(() => {
    const heading = state.presentation === "reading" ? readerElement.current?.querySelector("[data-reader-heading]") : null;
    scrollPositions.current.set(scrollKey, { top: window.scrollY, headingOffset: heading?.getBoundingClientRect().top });
  }, [scrollKey, state.presentation]);
  ...
  useEffect(() => {
    if (previousPresentation.current !== state.presentation && state.presentation !== "scene" && state.presentation !== "full") {
      const target = state.presentation === "reading" ? readerElement.current?.querySelector<HTMLElement>("[data-reader-heading]") : explorerElement.current;
      ...
      if (savedPosition !== undefined) {
        ...
        if (state.presentation === "reading") target?.focus({ preventScroll: true });
    ...
    if (state.presentation !== "scene" && state.presentation !== "full") {
  ...
  const scenePanel = <div className="explorer-visual">
    <div className="explorer-scene" aria-label="Interactive Pattern portrait">
      ...
      <div className="explorer-scene-toolbar" role="group" aria-label="3D controls" tabIndex={0} onKeyDown={(event) => {
        ...
        {state.presentation !== "scene" && <button ref={expandButton} disabled={!validMeshes || status === "unavailable"} className="explorer-expand" aria-label="Expand scene" title="Expand scene" onClick={() => present("scene")}><span className="explorer-mobile-icon" aria-hidden="true">⛶</span><span className="explorer-control-text">Expand scene <span aria-hidden="true">↗</span></span></button>}
      </div>
    </div>
    {chapterRail}
  ...
  const reader = <aside ref={readerElement} className="explorer-reader" aria-label="Chapter reading" onScroll={(event) => readerPositions.current.set(readerKey, event.currentTarget.scrollTop)}>
    ...
    </> : <div className="explorer-introduction"><p className="explorer-eyebrow">A portrait in four parts</p><ReaderHeading data-reader-heading tabIndex={-1}>A different way<br />to see your Pattern.</ReaderHeading><p>Each object belongs to a chapter of your reading. Choose one to explore its tensions, resources, and another expression.</p><p>The objects are a way into the words. Your complete reading is always here.</p><button className="explorer-primary" onClick={() => select(manifest.chapters[0].id, true)}>Explore the first chapter <span aria-hidden="true">→</span></button><p className="explorer-intro-note">Rotate the portrait to see every side, or unfold it to give each chapter space.</p></div>}
  </aside>;
  const Root = embedded ? "section" : "main";
  const fullReadingControl = <button ref={fullReadingButton} onClick={() => state.presentation === "full" ? back() : present("full")}>{state.presentation === "full" ? "Return to portrait" : "Full reading"}<span aria-hidden="true"> ↗</span></button>;
  return <Root onFocusCapture={(event) => { lastFocused.current = event.target as HTMLElement; }} ref={explorerElement} id="portrait-start" tabIndex={-1} aria-label="Pattern portrait explorer" className={`portrait-explorer${embedded ? " explorer-embedded" : ""} explorer-presentation-${state.presentation}`}>
    ...
    {state.presentation === "full" ? <CompleteReading manifest={manifest} embedded={embedded} /> : <>
      {embedded ? <div className="explorer-title"><h2>Your 3D portrait</h2><p>Four chapters, connected.</p></div> : <div className="explorer-title"><div><p className="explorer-eyebrow">Your Pattern</p><h1>{manifest.chapters.length === 4 ? "Four chapters. One portrait." : "Your Pattern."}</h1></div><p>Explore what connects you.</p></div>}
      {manifest.uncertainty && <p className="explorer-uncertainty">{manifest.uncertainty}</p>}
      <div className="explorer-mobile-modes"><button aria-pressed={state.presentation === "explore"} onClick={() => state.presentation !== "explore" && back()}>{state.presentation === "reading" ? "Return to portrait" : "Explore"}</button><button ref={readButton} aria-pressed={state.presentation === "reading"} disabled={!chapter} onClick={() => present("reading")}>Read chapter</button></div>
      <div className={`explorer-workspace${selected.length === 2 ? " explorer-is-comparing" : ""}`}>{state.presentation !== "scene" && scenePanel}{reader}</div>
      {state.presentation === "scene" && <Modal label="Expanded portrait scene" className="explorer-expanded-dialog" onClose={back}><div className="explorer-dialog-heading"><h2>Portrait scene</h2><button onClick={back}>Close expanded scene</button></div>{scenePanel}</Modal>}
    </>}

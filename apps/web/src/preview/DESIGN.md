...
The original portrait direction established the reading and interaction shell. The approved four-image direction supersedes the earlier shared bands and single-door study. The [delivery handoff](../../../../docs/superpowers/2026-09-05-pattern-portrait-handoff.md) owns current verification and delivery claims.
...
Production integration begins at [`PatternPortrait`’s `source` prop](../components/PatternPortrait.tsx): the account surface ([`AccountPatternPortrait`](../components/AccountPatternPortrait.tsx)) supplies an authorized published `PatternResponseV7`, or an explicit loading/unavailable state. This preview remains the fictional harness; authenticated storage, delivery, and deployment truth live with the account surface, not here. The [local manifest](../lib/pattern-portrait.ts) projects reader-visible content only. Its revision uses schema version, Pattern ID, and generation time; chapter identity is document-local ordinal.

-- Admit the Daily `reading`/`publisher` coordinate on the Codex provider
-- control plane. Forward-only; 0013 and 0014 are applied and immutable.
--
-- SQLite cannot alter a CHECK constraint, and the three constraints that have
-- to move are all on `codex_provider_jobs`, so the parent is rebuilt. Its only
-- child, `codex_provider_response_uploads`, carries ON DELETE CASCADE, so it is
-- staged into a plain table with no foreign key of its own, the parent and
-- child are dropped in dependency order, and the child is rebuilt and restored
-- from the staging copy. A cascade must never be the thing that deletes an
-- audit inventory.
--
-- Nothing else changes. No prompt or response content column is added -- those
-- bytes live only in create-only AES-GCM R2 envelopes -- no AEAD or KEK version
-- moves, and no ciphertext is rewritten. Every lifecycle, envelope, lease,
-- nonce, hash, safe-failure, uniqueness, and timestamp constraint below is the
-- 0013 text unchanged; only the three relationship CHECKs are widened.
--
-- The whole file is one D1 transaction, so an armed assertion probe rolls back
-- everything including the drops. The probes that CAN run before a destructive
-- statement do: a count that is already wrong is not worth discovering after
...
CREATE TABLE codex_provider_response_uploads_staging (
...
INSERT INTO codex_provider_response_uploads_staging (
...
WHERE (SELECT COUNT(*) FROM codex_provider_response_uploads_staging)
...
WHERE EXISTS (
  ...
  WHERE type = 'table'
    AND name NOT IN (
      ...
      'codex_provider_response_uploads_staging',
      'codex_provider_jobs_reading'
    )
...
-- ---------------------------------------------------------------------------
CREATE TABLE codex_provider_jobs_reading (
...
-- ---------------------------------------------------------------------------
INSERT INTO codex_provider_jobs_reading (
...
WHERE (SELECT COUNT(*) FROM codex_provider_jobs_reading)
...
ALTER TABLE codex_provider_jobs_reading RENAME TO codex_provider_jobs;
...
SELECT job_id, lease_token_hash, object_key, created_at
FROM codex_provider_response_uploads_staging;
...
WHERE (SELECT COUNT(*) FROM codex_provider_response_uploads)
   <> (SELECT COUNT(*) FROM codex_provider_response_uploads_staging);
...
-- ---------------------------------------------------------------------------
DROP TABLE codex_provider_response_uploads_staging;

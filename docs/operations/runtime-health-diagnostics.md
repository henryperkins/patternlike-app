...
## Candidate revalidation is a different GET

The `pattern` CLI subcommand above calls only diagnostics. It never opens keys
or artifacts and never replays a writer response. There is no
`runtime-health.ts` subcommand for candidate revalidation.

To explain a terminal `candidate_invalid` result without returning private
prose, issue one GET under the same Access auditor boundary:

```text
/admin/pattern-generations/{generation_id}/candidate-revalidation?purpose=incident_response
```

The route records the scoped access decision — including the intended
artifact-class list — before opening any key or artifact. A grant is an access
decision, not proof that replay succeeded. It selects only the exact final
completed writer coordinate, returns provenance hashes and closed failure
codes, and does not unstick, retry, regenerate, or publish. Procedure and
fail-closed outcomes: [`docs/deploy/pattern-candidate-revalidation.md`](../deploy/pattern-candidate-revalidation.md).

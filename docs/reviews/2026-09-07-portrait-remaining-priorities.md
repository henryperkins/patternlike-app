...
| --- | --- | --- | --- |
| 1 — R5 | P2 | In ordinary single-chapter exploration, the selected annotation replaces the visible chapter title with a facet/passage chip. Facet feedback remains limited. | Keep the chapter name visible with secondary facet/passage information; strengthen source-bound emphasis and the exact scene/reader passage loop. Preserve comparison identities, camera position while reading, occlusion, keyboard access and the four-label budget. |
| 2 — R6 | P2 | The desktop reader scrolls internally, while image/compare actions follow its prose; more content and actions are easy to miss. | Make continuation and actions apparent, using an accessible affordance or footer. Keep all prose readable, focused controls unobscured, and the originating passage and scroll position on return. |
...
R5 and R6 are the next interaction priorities. Detailed evidence, scope and acceptance criteria appear under **Remaining interaction work** below. P3 evidence and completion criteria appear under **Small follow-ups**. Guide progress through facets and whether a guide inherits desk/layout choices remain optional onboarding evaluations, not confirmed defects or release blockers.
...
**Status: open, P2.** In ordinary single-chapter exploration, the selected visible title is replaced by an annotation chip such as “● Tensions · 1.” Its accessible name still includes the chapter, and the exact passage link works. The current label branch in [PortraitScene.tsx](../../apps/web/src/components/portrait-explorer/PortraitScene.tsx) renders the chapter-title span inside an annotation only when comparing. R3 already preserves both comparison names; that behavior must remain intact. The renderer uses `facet` for annotation content without selecting a different pose or mesh-feature emphasis for each facet.
...
**Status: open, P2.** The original account Overview reader measurement was 722px high with 938px of content; its image/compare action row began below the visible reader boundary. Those dimensions are historical. Current source still places `.explorer-reader-actions` after the chapter text in [PortraitExplorer.tsx](../../apps/web/src/components/portrait-explorer/PortraitExplorer.tsx), and ordinary desktop/account readers still use bounded internal scrolling in [explorer.css](../../apps/web/src/components/portrait-explorer/explorer.css). The settled R4 desktop capture also cuts through the action row. R4 browser checks confirmed image inspection and comparison work; the open issue is discovering the continuation and actions.

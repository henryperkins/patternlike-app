...
**Known gap.** Tasks 5, 6, and 7 change request-handling behaviour that the current test setup cannot exercise — `apps/api` has no Miniflare or `wrangler dev` harness, so there is no integration test binding a real D1. The unit tests cover the pure validation and configuration logic; the D1 constraint changes are covered by `smoke_check.py` against in-memory SQLite. Standing up `@cloudflare/vitest-pool-workers` for genuine end-to-end route tests is the obvious next step and is **not** in this plan.

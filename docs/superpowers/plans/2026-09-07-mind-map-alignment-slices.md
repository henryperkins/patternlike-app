...
**Reconciled:** 2026-09-09 with the roadmap's four opportunity directions and source `d338b86c9444ebe2f372f2a2f3990f4a1270bc80`. Interpretation quality and one connected journey remain the first product investigations. Feedback timing, permission consequences, diagnostics, and measured fairness extend those foundations. Local three-to-six-chapter observatory support already exists; broader generated artwork is a conditional extension. Earlier evidence below retains its original dates.

**Status (2026-09-09):** Slice 1 is locally implemented in the isolated worktree; its [execution record](../archive/plans/2026-09-09-source-map-maintenance-implementation.md) retains the results and limits. Slice 2 is withdrawn. Slice 3’s offline assessment, chart panel and approved mapping/prose-check fixes are locally complete; human adjudication remains open. Slice 4 was separately merged and deployed in [PR 52](https://github.com/henryperkins/patternlike-app/pull/52), with migration 0030 applied first and runtime identity verified. Slice 5A/5B software is locally implemented and verified, with migration 0031 undeployed and new generation effects disabled by default. Slice 6 receives credit for Slice 4’s exact-chapter continuity; its broader rendered recovery matrix remains open. Human comprehension and measured effects remain open. Slices 7–12 are planned; 13 is conditional.

## Current integration — 2026-09-11

The [approved integration sequence](../archive/plans/2026-09-11-roadmap-integration.md) recovers Slices 1/3/5 onto main's `69a4f78`, preserving the feedback-draft correction. Source corrections and optional map tools, unsigned quality candidates and safety 1.0.1, categorized feedback, initial Pattern retry eligibility, and sequential runner fairness are now implemented together in focused commits on `codex/roadmap-integration`. The [September 11 audit](../../reviews/2026-09-11-roadmap-progress-and-drift.md) is the historical starting assessment; its “unmerged” findings describe the state before this integration.

The categorical loader examines at most the newest 100 SQL-matching candidates in deterministic order; exact target/grant/category checks precede the reading engine's existing shared 20-feedback admission cap. Candidates beyond that window remain unexamined. New effects stay disabled by default and still require the paired selection/prompt versions. Signing/activation of ontology 0.1.2 remains unselected.

The user selected independent AI primary/secondary editorial reviews and simulated comprehension on September 11. Their [completed four-output assessment and reconciliation](../../reviews/artifacts/interpretation-quality/2026-09-11-ai-review/README.md) supplement software evidence; they do not certify human benefit or fill historical participant fields. The technical observatory matrix covers existing 3–6 chapter reading stations; wider generated artwork remains unselected. Slice 7's composed reader presentation/consequences, bounded metrics, scoped diagnostics and inactive policy evaluator are implemented in the [readiness follow-up](../../reviews/2026-09-11-readiness-metrics-results.md); alert adoption and unsupported diagnostic domains remain open. Slice 8's persistent 4:1:1 rotation is implemented; installed-runner adoption remains separate. Existing lease, spend and concurrency limits remain intact. The [integration results and AI walkthrough](../../reviews/2026-09-11-roadmap-integration-results.md) record local evidence. Follow [PR 54](https://github.com/henryperkins/patternlike-app/pull/54) for actual merge, migration and deployment outcomes rather than reusing dated observations below.
...
## Order and verification scope

| Slice | Start condition | Completion handed to later work |
| --- | --- | --- |
| 1. Source truth and map maintenance | Locally implemented | Corrected evidence index and optional map tools |
| 2. Verification expansion — withdrawn | No implementation required | Existing verification commands retained; no dependency for other slices |
| 3. Interpretation-quality baseline | Offline assessment complete; human/prose review open | Concrete mapping findings, 60-fragment/40-record assessment, twelve-case panel; adjudication pending |
| 4. Connected reader journey and provenance | Software deployed; signed-in production journey and human comprehension unobserved | Exact owned editions, encrypted passage support, and complete connected navigation |
| 5. Feedback improvements | 5A/5B software verified locally; rollout and effects evaluation remain open | Responses at the point of reading, compatible categories, clear eligibility/retention, and a versioned compiler disabled by default |
| 6. Observatory continuity; conditional artwork expansion | Use existing 3–6 chapter stations and 4's exact source identity; decide artwork expansion separately | Supported entry/return navigation and preserved fallback; v2 generation only if its separate entry decision is met |
| 7. Readiness and operational visibility | Agree state composition, permission consequences, safe status fields, and response ownership | Reader actions/consequence explanations, operator diagnostics, and queue/failure visibility |
...
Slices 3 and 4 proceed alongside each other; the journey prototype does not wait for completed editorial certification. Slices 6–8 can proceed independently once their shared interfaces are recorded. Existing 3–6 chapter reading/navigation, the connected journey, readiness, and runner fairness do not wait for Slice 6's conditional artwork schemas, migration, or producer enablement. Slices 9–12 do not gate reader-facing work and need not run in numeric order. Slice 13 is conditional.

Use existing verification commands and focused regression tests for changed behavior. Do not add gate stages, release wrappers, mandatory map checks, receipt formats, or approval workflows as part of these slices. Map capture/check and `test:source-map` remain optional maintenance tools outside `test:content`.

The user has said the full local gate is unnecessary for this local implementation work. The existing repository merge policy above applies when a merge is actually undertaken. Report what changed, the checks actually run, and any relevant limits; this ledger adds no separate release-state tracking requirement. Deployment or operational success still requires observing the relevant operation.

## Detailed specification coverage

The following designs settle the major implementation interfaces and verification boundaries. They are specifications for review, not implementation, merge, spending, migration, or deployment approval. Use each slice’s acceptance criteria for its own scope; no additional release record format is required.

| Slices | Specification | Decisions made concrete |
| --- | --- | --- |
| 1 | [Source truth and map maintenance](../specs/2026-09-07-source-truth-map-maintenance-design.md) | Dated original/corrected map inputs, scope and correction inventory, source manifest, reproducible checks |
| 2 | [Verification expansion withdrawn](../archive/specs/2026-09-08-release-preflight-design.md) | Existing commands and focused tests; no new implementation or dependency |
| 3 | [Interpretation-quality baseline](../specs/2026-09-08-interpretation-quality-baseline-design.md) | 60-fragment/40-record review, source/predicate/prose diagnosis, twelve-chart panel, controlled comparison and adjudication |
| 4–5 | [Reader relationships and feedback](../specs/2026-09-08-reader-relationships-feedback-design.md) | Explained branching navigation, exact editions/cycles, passage support, timely responses, bounded grants and separate expiry/retention |
| 6 | [Observatory continuity and conditional artwork expansion](../specs/2026-09-08-adaptive-observatory-contract-design.md) | Existing 3–6 stations and exact entry/return; conditional v2 negotiation, migration, consent and rollout |
| 7–8 | [Readiness and runner fairness](../specs/2026-09-08-readiness-runner-fairness-design.md) | Composed states/actions and consequences, scoped diagnostics, safe metrics/clocks, testable 4:1:1 scheduling |
...
Include the corrected map's plain and anchored variants under `output/mindmaps/2026-09-09-corrected/` as separate dated inputs. Their 129 anchors across 100 files are an architecture-overview index, not a complete coverage inventory or a replacement for the original 86-file snapshot. Preserve both histories and state each input's known source/capture limits.
...
- Reconcile health/meta guard exceptions, the deletion-receipt authority, per-command offline/live harness behavior, revision purpose versus retry origin, and chart-specific portrait automation. Show existing 3–6 chapter reading separately from optional four-chapter artwork.
...
**Verification:** Reproduce the corpus preparation/builder result offline and compare canonical content identity with the dated record, without signing or ingestion. Check Markdown links and line anchors. Exercise snapshot checks in a temporary fixture repository with unchanged files, changed dirty bytes under the same HEAD, missing files, stale anchors, and edited map inputs. Verify that documentation/comment changes preserve executable behavior and do not accidentally change generated Pattern source identity. Run focused tooling tests when changing the optional map tools.

**Release boundary:** Documentation and local tooling only. No corpus amendment, generation, activation, deployed-variable change, or reading migration is part of this slice. Map snapshots are optional source-reference artifacts, not release prerequisites.

**Responsible role / next handoff:** Repository maintainer. Hand the corrected inventory to 3/4; map maintenance does not block them.

## Slice 2 — Verification expansion withdrawn

**Decision:** The user withdrew the proposed release-preflight expansion on 2026-09-09 before implementation. See the [superseding decision](../archive/specs/2026-09-08-release-preflight-design.md).

No new release command, summary sidecar, candidate-tree comparison, PR check, remote observation, or compatibility-record format will be implemented for this slice. Existing commands and repository merge policy remain in place. There is no Slice 2 prerequisite for the remaining work.

**Next handoff:** Interpretation quality (3) and the connected reader journey (4).
...
**Progress (2026-09-09):** [Offline baseline](../../reviews/artifacts/interpretation-quality/2026-09-09-baseline/README.md) covers all 60 fragments and 40 admitted records, twelve chart cases, and six synthetic controls. It confirms 17 proposition fallbacks in counter-expression fields and missing applicability conditions for six meanings. The approved [contrast extraction fix](../../reviews/artifacts/interpretation-quality/2026-09-09-counter-expression-fix/README.md) is locally implemented in a new unsigned 0.1.1 candidate: all 38 explicit counter-expression labels plus Mars’s counterweight are selected, and proposition fallbacks fall from 17 to zero. The approved [applicability fix](../../reviews/artifacts/interpretation-quality/2026-09-09-applicability-fix/README.md) is also locally implemented as unsigned 0.1.2: concentration requires stellium, five unsupported meanings are omitted, and exact-time uncertainty retains source-verbatim methodology. Twelve same-case selections were compared, including the existing synthetic word-count failures. Provider comparison results are recorded with that fix. Human editorial adjudication and reader comprehension remain open; the full quality baseline is not marked complete.
...
**Retained-prose follow-up:** The [diagnosis and replay](../../reviews/artifacts/interpretation-quality/2026-09-09-retained-prose-diagnosis/README.md) confirm that the 245-word exact-before chapter correctly fails the unchanged 250-word bound. The exact-after failure is a lexical false positive for “You do not have to predict what…”, corrected locally under publication-safety 1.0.1 with narrow regression coverage. All original comparison artifacts are preserved. No new provider call, ontology activation, or safety-policy deployment occurred. Human evaluation can use the [prepared worksheet](../../reviews/artifacts/reader-feedback/2026-09-09-implementation/human-review-worksheet.md); no reviewer results have been recorded.

**Entry decisions:** Name the editorial reviewer and evaluation owner; freeze rubric dimensions, panel cases, scoring anchors, and escalation rules before judging outputs. Begin the inventory and reuse existing evaluation helpers while reviewer assignment is outstanding, but do not claim adjudicated certification.
...
- Attribute weak distinctions to source material, admitted predicates, or generated prose only when evidence supports that diagnosis. Controlled follow-up comparisons preserve the baseline and disclose changed inputs/pins; artwork fidelity remains a separate measure.
...
**Release boundary:** Rubric, register, and evaluation report using existing helpers. Any small report helper remains optional and outside existing gate commands. Any new corpus release, predicate grammar, prompt/policy change, provider spending, or reader recruitment is a separate scoped action. No production activation is needed for the baseline.
...
**Progress (2026-09-09):** The [application implementation and release record](../archive/plans/2026-09-09-reader-journey-production-implementation.md) connect an actual Daily paragraph through owner-authorized exact Pattern/Timing/saved-Daily reads and existing feedback, with a direct Timing branch. Accepted passage facts are retained encrypted in the existing publication transactions; migration 0030 includes owner/document lifecycle references. Account erasure, rotation, source replacement and replay preserve the existing privacy boundary. The observatory opens exact chapters for three through six chapters without artwork or generation. Focused encrypted-D1, resolver, contract, component and rendered checks cover identity, access, support gaps, navigation and existing response behavior. The earlier [fictional preview](../../../apps/web/reader-journey.html) remains available separately. Production migration 0030 preceded the compatible PR 52 deployment; public metadata confirms the merged SHA and deployed version. A signed-in production journey and recorded human comprehension exercise remain unobserved.
...
**Entry decisions:** Recorded in the approved specification and the additive reader-relationships-v1 contract package. The first resolver admits shared calculated features, shared natal participants, and dated occurrences; editorial relations and reader reflections require later explicit support.
...
- Support branching from a passage explanation to a justified Pattern chapter or Timing connection. Entry into an existing observatory station and return to the origin preserve exact edition/chapter identity and navigation position, with no generated-artwork prerequisite.
...
**Verification:** Component tests for the four destinations and relationship states; API/access tests for edition and permission handling; rendered browser checks with screenshots at narrow and wide viewports. Test source change/recall, grant revocation, uncertainty, and deletion. Run the affected API/web tests.

**Release boundary:** Slice 4 was isolated from local Slice 3/5 and map work, passed the existing full merge check with its summary pasted into PR 52, and deployed after migration 0030. Existing editions without retained support remain explicit gaps. Production identity/liveness and unauthenticated rejection were observed; signed-in navigation and human comprehension remain separate open evidence. No graph database, provider request change, new model, or new verification stage was added.
...
**Progress (2026-09-09):** The approved [implementation](../archive/plans/2026-09-09-reader-feedback-implementation.md) is locally complete. Slice 5A offers existing feedback on Today alongside check-in; 5B adds exact-edition categorical options/receipts and birth-correction navigation. Encrypted event/note/replay storage follows USR-12’s 24-calendar-month limit and participates in export, rotation, deletion, restore replay and bounded expiry cleanup. The new selection 1.2.0 / prompt 1.0.4 pair preserves old command behavior and admits only closed evidence-derived category signals; notes are excluded. The compiler defaults off. Current V5 records have no supported theme associations, so relevance feedback is recorded without a ranking effect. [Browser evidence](../../reviews/artifacts/reader-feedback/2026-09-09-implementation/README.md) covers Today/History and separate check-in interaction; the human worksheet is unfilled. Migration 0031, client rollout, activation and measured benefit remain open.

**Scope and deliverable:** Optional responses at the point of reading and distinct feedback categories with understandable, permitted effects, building on 4. Extend the existing [feedback store](../../../apps/api/src/db/feedback.ts), [context compiler](../../../apps/api/src/services/context-compiler.ts), [ranking policy](../../../packages/reading-engine/src/ranking.ts), and reader controls. Preserve the separate purpose of a check-in for future context.
...
- Today can offer a response to its exact edition without requiring a History reopen or replacing its check-in. Explain check-in freshness, stored-data retention, context permission, and actual passage use separately. The proposed seven-day categorical-effect window is neither the check-in's 24-hour freshness window nor a deletion deadline; event retention must be explicit before release.
...
**Verification:** Extend [feedback integration tests](../../../apps/api/src/routes/feedback.integration.test.ts), context-compiler tests, and reader interaction tests. Cover no grant, revocation, duplicates, old editions, birth correction, note deletion/export, and unsupported old/new wire values. Run contract tests when contracts change, plus affected tests and rendered checks.

**Release boundary:** Apply migration 0031 before compatible Slice 5 deployment; it has not been applied to production. Preserve old resonance and queued export contracts. Category storage/client rollout and activation of the new prompt/selection pair are separate states; activation requires matching configuration and the existing permitted uses. Local focused checks introduce no new verification stage.
...
## Slice 6 — Observatory continuity and conditional artwork expansion

**Specification:** [Existing chapter support, connected navigation, and conditional v2 artwork](../specs/2026-09-08-adaptive-observatory-contract-design.md).

**Current evidence:** Slice 4 already delivers the immediate exact-chapter entry/return implementation, 3/4/5/6 component behavior, complete-text fallback, and a desktop/mobile rendered journey. Credit this work here rather than implementing it again. The full rendered count/recovery matrix and human comprehension exercise remain open. Conditional v2 artwork has not been selected or implemented.

**Scope and deliverable:** Extend supported entry/return navigation through the existing 3/4/5/6-chapter observatory while preserving full readings and optional-asset fallback. Coordinate [account presentation](../../../apps/web/src/components/AccountPortraitExplorer.tsx), [manifest creation](../../../apps/web/src/lib/pattern-portrait.ts), and Slice 4's exact-edition relationships. A separate, conditional extension can widen [API portrait admission](../../../apps/api/src/services/pattern-portrait.ts) and [mesh generation](../../../apps/api/src/services/pattern-portrait-mesh.ts) beyond four chapters under the retained v2 design.

**Entry decisions:** Local adaptive stations, comparison, guided reading, and passage navigation already exist in the September 9 source. Select supported journey entry/return cases and reuse current geometry and source identity. Before any v2 artwork work, record whether its expected reader value justifies new schemas, migrations, count-dependent provider work, and explicit consent. The existing reading experience and other product slices do not depend on that decision.
...
- Journey links select the exact source chapter and return to the originating edition/paragraph with focus and reading position restored. Unsupported or inaccessible destinations remain honest unavailable states; no regeneration or artwork opt-in occurs merely by following a link.
...
- If artwork expansion is taken up, freeze count/protocol identity, preserve v1 assets/leases, negotiate capability before claiming work, and satisfy the spec's migration, consent, and rollout order. Retain four mesh inspection views independently of chapter count.

**Verification:** For navigation, test all four chapter counts, exact entry/return identity, missing assets, source changes, and fallback; capture rendered desktop/mobile, reduced-motion, keyboard, and recovery evidence. Compare return-navigation/comprehension outcomes with the journey baseline. Conditional artwork work additionally requires the full count/protocol, migration, image/mesh identity, and cancellation matrix. Run each implemented unit's affected tests; mark physical-device or assistive-technology checks separately.

**Release boundary:** Record connected navigation separately from conditional artwork-contract support, runner/client adoption, and v2 producer enablement. Navigation can complete using existing folios/v1 assets without new provider work or an artwork migration. If expansion proceeds, stage compatibility before emitting new shapes, keep accepted assets readable, and retain forward recovery/rollback. Existing station support is a verified source premise, not evidence that these new deliverables are complete.
...
**Scope and deliverable:** Consistent reader explanations/actions and permission consequences plus safe operator metrics/diagnostics, using [Pattern state](../../../apps/api/src/services/pattern-state.ts) and the other existing domain authorities. Include queue age and failure visibility by text, portrait, and mesh work class.
...
- Explain known before/after effects on saved content, unfinished work, and future generation at existing permission/correction/deletion controls. Retain unknown states and separate request acceptance from completed erasure/cancellation; do not change policies through presentation.
...
- The read-only diagnostic view identifies a blocker only from observed, authorized state. Keep aggregate health free of private identifiers and use separate existing purpose-audited records for scoped inspection. Report missing stages as uncollected/unavailable; measure identification/recovery time separately from queue completion latency.
...
**Verification:** State-service and UI mapping tests; fake-clock queue/lease tests; redaction tests using hostile/private error inputs; authorization tests for operator status. Render representative states and actions. Run the affected tests. Operational completion additionally requires dated evidence that the chosen destination/responder receives the intended signal.

**Release boundary:** State/consequence presentation, metrics, diagnostic presentation, and alert wiring may ship independently with their evidence boundaries explicit. Existing v1 measurement does not wait for Slice 6's conditional table reconstruction; coordinate preservation if both migrations are pursued. An unassigned responder leaves operational adoption open, while locally verified engineering can still be recorded.
...
**Entry decisions:** Reproduce text-first starvation using deterministic fake clients, then evaluate the spec's concrete 4:1:1 weighted rotation with concurrency one against that baseline. Preserve its six-slot opportunity bound and explicit in-flight/timeout assumptions; the weights are not claimed to be production-optimal. Reuse 7's metric definitions; do not wait for its UI or optional artwork expansion.
...
**Verification:** Runner fake-clock/fake-client regressions for continuous text load, mixed backlogs, no work, each failing class, shutdown, expired leases, cancellation, and duplicate completions. Run `npm run test -w @patternlike/codex-runner`, affected API claim/lease tests, and runner build.
...
**Verification:** Characterize the two adapters with fixed requests and fake exchange outcomes before extraction. Retain adapter, contract, job, artifact, and cancellation tests; run the affected API tests. Live provider calls are unnecessary for a mechanical extraction.
...
**Verification:** Characterization/integration tests cover crash-after-artifact adoption, exhausted attempts, writer/verifier corrections where applicable, lease/consent loss, duplicate queue delivery, failed publication, and terminal cleanup. Retain existing stage-protocol tests, run the affected tests.
...
**Verification:** Configuration resolution/guard tests for old/new/both/missing/invalid values; frozen command and historical receipt fixtures; release-evidence/reconciliation tests; source fingerprint checks and build. Run the affected tests for each compatible stage.
...
**Verification:** Lockfile dry run, workspace typecheck/test/build, deterministic calculation goldens, API calculation-client tests, packaging/container build from the documented context, and source-offer path/license parity checks. Update release-lane tests if the workspace name changes their expected summary. Use the existing commands with the renamed workspace; preserve gate coverage without adding stages.
...
Prepare a concrete task plan only when needed for the slice being implemented. Use the smallest useful change, resolve its relevant entry decisions, and preserve unrelated work. Extend existing regression cases for changed behavior and run the affected checks; include rendered or editorial review where the deliverable needs it.

Report the change, actual verification results, and relevant limits in the normal implementation summary or PR. This ledger requires no additional checklist, receipt, approval workflow, or gate. Record release and production results only when those actions were authorized and performed.
...
On 2026-09-09, the six linked specifications and this ledger were reconciled with the updated roadmap using source `d338b86c9444ebe2f372f2a2f3990f4a1270bc80`. Targeted checks covered current corpus mapping, reader input and retention, local observatory support, portrait automation, route authority, release-tool execution boundaries, and runner ordering. Original source/build/production observations remain dated. Documentation checks cover local links/anchors, structure, whitespace, and changed-file scope; no application implementation, provider evaluation, production action, or new runtime test result is implied by this reconciliation.

On 2026-09-09, the user withdrew the proposed Slice 2 verification expansion and requested simpler verification throughout the roadmap. The source-map tests were removed from automatic `test:content` execution and retained as an optional command. Repeated per-slice full-gate mandates and the additional release-record checklist were removed. Earlier verification results above remain historical.

# Slice 6: adaptive Observatory contracts and staged compatibility

Date: 2026-09-08

Status: specification for review; no schema, migration, asset, or implementation changed.
...
Source review: commit `6e706741f03253f2807d33380afb529161f3481f` plus the pre-existing local Observatory edits, inspected on 2026-09-08. Preserve and reconcile those edits when implementation begins; this document does not treat them as merged or deployed.

## Decision and source boundary

Use adaptive stations for every valid three-, four-, five-, or six-chapter Pattern. Each source chapter has one station, in source order. Preserve every chapter field and the complete reading. Do not force four chapters, invent missing chapters, concatenate chapters, or choose an undisclosed subset.

The current limitation spans [portrait admission/jobs](../../../apps/api/src/services/pattern-portrait.ts), [client manifest loading](../../../apps/web/src/lib/pattern-portrait.ts), [shared types](../../../packages/shared/src/portrait-types.ts), graph generation, image/mesh claim schemas, and D1. Both [portrait image jobs](../../../db/d1/0026_pattern_portraits.sql) and [mesh jobs](../../../db/d1/0027_portrait_mesh_automation.sql) restrict `chapter_index` to 0–3. Existing completion/readiness code also compares against four. Changing the renderer alone cannot close this slice.

Keep existing v1 responses, accepted image/model/graph bytes, per-chapter source serialization, and historical provenance readable. New support uses versioned contracts and explicit compatibility dispatch; do not widen a frozen v1 schema or retag accepted v1 bytes as newly generated v2 material.
...
For four chapters, retain the existing station geometry and v1 rendering behavior. For three/five/six, use deterministic layout `adaptive-stations/v1`: chapter i has angle `pi / 4 + 2 * pi * i / N` and ground position `[R * cos(angle), 0, R * sin(angle)]`. Define R as the larger of the existing four-station radius and `(2 * footprint_radius + clearance) / (2 * sin(pi / N))`. Derive the footprint from the station's actual accepted display bounds, including its reader desk; use the largest footprint for all stations and the existing comparison clearance. Keep those geometry inputs versioned with the layout and include every station in world/camera bounds. Preserve the reading entrance and selected-station focus; visual acceptance may revise this proposed layout before its policy is frozen.
...
Capture rendered desktop/narrow/landscape states for each count, keyboard and reduced-motion navigation, long chapters, read-position restoration, missing assets, non-WebGL mode, and context loss/recovery. Reconcile the pre-existing local Observatory edits before touching those files. Run affected shared/API/runner/web suites, contracts, fixture asset checks, fingerprint/build checks, and the full local gate on final source.

Slice 6's contract/storage/compatibility work, runner/client adoption, and adaptive producer enablement are separate release boundaries. Local acceptance requires the full count matrix and preserved reading/source identity; operational completion additionally requires compatible installed/runtime evidence and an authorized lifecycle result.

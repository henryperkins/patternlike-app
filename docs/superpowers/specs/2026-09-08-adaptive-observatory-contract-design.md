...
Status: immediate reader continuity is substantially implemented through Slice 4, now separately deployed (PR 52). On September 11, 2026 the owner selected the generated-artwork v2 expansion: images must cover every chapter, including Patterns longer than four chapters. The all-chapter artwork implementation merged in PR 56; producer enablement is owned by the [adaptive artwork runbook](../../deploy/adaptive-portrait-artwork.md). Deployment, runner installation, production migration and producer enablement remain separate operations. Signed-in production journey observation and human comprehension review remain open.

...
Status (2026-09-11): Slice 8's weighted sequential policy is implemented on the roadmap integration branch, with deterministic progress/cooldown/lease regressions. Initial Pattern failure actions now require explicit server retry eligibility and retain read-only refresh. The broader Slice 7 presentation selector, metrics and diagnostics below remain proposed. Runner installation/adoption is not implied by this implementation.

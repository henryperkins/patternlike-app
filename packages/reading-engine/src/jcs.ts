 ...
 *
 * Used for the two `patternlike.*-id.v1` hash preimages and nothing else. The
 * repository's legacy `canonicalJson` (JSON.stringify over sorted keys) stays
 * exactly as it is: M0 release, object, and signature hashes are byte-frozen on
 * its behaviour, and reinterpreting them as JCS would invalidate every stored
 * bundle. Naming and versioning that legacy profile is still an open escalation.
 *
 ...
 * (orbs, weights, speeds, minute counts), so an implementation here would be
 * untested code sitting inside a hash preimage. Rejecting keeps the failure

...
function measure(image: PortraitImagePixels, index: number): Evidence {
...
/** Reject an unusable chapter before its generated image becomes immutable. */
export function isUsablePortraitImage(image: PortraitImagePixels): boolean {
  try {
    measure(image, 0);
    return true;
  } catch {
...
export function createPortraitGraph(images: readonly PortraitImagePixels[], sunSign: ZodiacSignName | null = null): PortraitGraph {
  ...
  return {
    engine_version: PORTRAIT_ENGINE_VERSION,
...
export function isPortraitGraph(value: unknown): value is PortraitGraph {
  ...
  if (count < 4 || count > MAX_STARS * 4 || positions.length !== count * 3 || strengths.length !== count
    || connections.length < count - 1 || connections.length > count * 3 || color.length !== 3 || contributions.length !== 4
    || !positions.every((n) => finite(n) && Math.abs(n) <= 10)
    || !sources.every((n) => Number.isInteger(n) && n >= 0 && n <= 3)
  ...
  return contributions.every((item, index) => {
    ...
    return c.index === index && c.stars === counts[index]
